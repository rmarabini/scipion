...
# **************************************************************************
"""
This module contains the protocol for 3d classification using a hierarchical approach with relion.
"""
...
from pyworkflow.em.packages.relion.convert import relionToLocation, rowToAlignment, isVersion2
...
#Comments: 
# En _setNormalArgs:
# '--i': self._getFileName('input_star'), 
...
class ProtRelionHierarchicalClassify3D(ProtClassify3D, ProtRelionBase):
    """    
    Protocol to classify 3D using Relion using a Hierarchical appraoch.
    """
    _label = '3D hierarchical classification'
    ...
    def _initializetRelionArgs(self):
        """ Prepare the command line arguments before calling Relion. """
        # Join in a single line all key, value pairs of the args dict        
        if self.doContinue:
            self._setContinueArgs(self.args)
        else:
            self._setNormalArgs(self.args)
        if isVersion2():
            self._setComputeArgs(self.args)
                    
                    
    def _insertRelionStep(self):
        """ Convert args to a string and launch Relion. """
        params = ' '.join(['%s %s' % (k, str(v)) for k, v in self.args.iteritems()])
        if self.extraParams.hasValue():
            params += ' ' + self.extraParams.get()        
        self._insertFunctionStep('runRelionStep', params)

    def _updateArgs(self):
        """Modify args values to aluch relion with new parameters """
        self.args.update({'--o': self._getExtraPath('test')})
